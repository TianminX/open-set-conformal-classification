...
# One-class-classifier open-set benchmark on the CelebA real data.
# The unknown score is the SAME one-class classifier the CGTC pipeline uses
# inside its feature-based XGT p-values (LocalOutlierFactor with
# n_neighbors=1, novelty=True, fit on the training-split embeddings), on top
# of the same KNN closed-set classifier as the CGTC pipeline (seen-class
# probabilities untouched). Two variants:
#   OCC (raw):   s(x) = LOF value / occ_scale, clipped to [0, 1], used
#                directly as the unknown probability (arbitrary level, like
#                the other raw scores)
#   Recal OCC:   the same score wrapped in GTRecalOpenSet so the average
#                unknown probability is anchored at the Good-Turing estimate
#                M1/n of the training split (out-of-sample mean from an
#                internal 20% holdout)
# Purpose: test whether the ranking information CGTC consumes (the OCC),
# combined with the Good-Turing level, closes the gap to CGTC-MM within the
# direct-conformalization construction, isolating the contribution of the
# framework from that of the novelty ranking.
#
...
#
# Usage:
#   python real_experiment_celeb_occ_recal.py n_ref n_test calib_num alpha_total \
#          n_label_total k_top k_bot batch_num

if len(sys.argv) != 9:
    print("Error: incorrect number of parameters.")
    print(
        "Usage: python real_experiment_celeb_occ_recal.py n_ref n_test calib_num "
...
# One-class-classifier unknown score. The OCC defaults inside OCCOpenSet to
# LocalOutlierFactor(n_neighbors=1, novelty=True), matching occ_choices['lof']
# in real_experiment_celeb_mm_plugin.py. occ_offset=1.0 subtracts the
# theoretical inlier LOF value (so inliers land near 0 and the multiplicative
# GT recalibration retains a real novel-vs-seen contrast); occ_scale=2.0 maps
# the shifted values into [0, 1] and is absorbed by the GT recalibration.
base_occ = black_boxes.OCCOpenSet(
...
# GT-recalibrated wrapper: unknown level anchored at M1/n (training split),
# out-of-sample score mean from an internal 20% holdout.
classifier_occ_recal = black_boxes.GTRecalOpenSet(
    base=black_boxes.OCCOpenSet(
        n_neighbors=n_neighbors,
        weights='distance',
        algorithm='auto',
        leaf_size=30,
    ...
    recal_frac=0.2, cap=0.9, random_state=42
)

# Methods to evaluate (random internal split only).
methods_list = {
    'Method (OCC)': base_occ,
    'Method (Recal OCC)': classifier_occ_recal,
}
...
def analyze_data_occ(X_ref, Y_ref, X_test, Y_test, methods_list,
                     alpha, calib_size, random_state=2024):
    """
    Run the raw and recalibrated OCC methods and evaluate prediction sets.
    ...
    For each method:
      ...
      - Compute joker-adjusted set sizes: whenever '?' is in a prediction set,
        ...
        those labels explicitly. Same accounting as the other benchmarks.
    """
    ...
    for method_name, classifier in methods_list.items():
        ...
        # The seen-class columns are the plain KNN probabilities (rescaled),
        # so top-k accuracy should match GT-KNN exactly; the p_open columns
        # show the feature-dependent unknown mass.
...
def run_real_experiment(X, Y, image_names, n_ref, n_test, num_exp, batch_num):
    """
    Run direct-conformalization experiments with the OCC-based classifiers.
    """
