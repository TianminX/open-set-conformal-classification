...
# Unified open-set benchmark runner on the CelebA real data (directly
# conformalized open-set classifiers, Algorithm "open-set plug-in" of the
# paper). One script covers every benchmark family so that all recalibrated
# variants share ONE recalibration procedure, selected by `mode`:
#   scale    - original multiplicative recalibration p_unk = min(c s, cap)
#   center   - subtract the median holdout score of pseudo-seen points, then
#              rescale (uniform generalization of the LOF inlier centering)
#   isotonic - isotonic estimate of P(novel | s) on the holdout, then rescale
# (see black_boxes.GTRecalOpenSet). In every mode the average unknown
# probability is anchored at the Good-Turing estimate M1/n of the training
# split, using training data only.
#
# Families (methods run in one job):
#   knn     : Recal KNN-dist (k=10), Recal KNN-MSP          (raw: celeb_knn_scores_raw)
#   knn1    : KNN-dist (k=1) raw + Recal KNN-dist (k=1)     (k adapted to 2-3 images/identity)
#   proser  : Recal PROSER                                  (raw: celeb_proser)
#   openmax : Recal OpenMax-MLP (simplified), Recal OpenMax-KNN  (raw: celeb_openmax)
#   occ     : OCC raw + Recal OCC for four one-class classifiers:
#               lof     - LocalOutlierFactor(n_neighbors=1), centered at its
#                         inlier value 1 and divided by 2 (as in celeb_occ)
#               iforest - IsolationForest(n_estimators=10, random_state=batch),
#                         the CGTC pipeline's configuration
#               ocsvm   - OneClassSVM(rbf, nu=0.05, gamma='scale'), the CGTC
#                         pipeline's configuration
#               ocsvm20 - OneClassSVM(rbf, nu=0.05, gamma=20): a kernel width
#                         comparable to the same-identity distance on the
#                         unit-norm FaceNet embeddings (the default bandwidth
#                         is far too wide for them)
#             iforest / ocsvm raw scores are mapped to [0, 1] by the 1st-99th
#             percentile range of out-of-sample scores from an internal split
#             of the training data (label-free 'auto' normalization in
#             OCCOpenSet); the OCSVM score is minus the log of its size-free
#             kernel average (see OCCOpenSet._raw_score).
# The seen-class component is the same KNN as the CGTC pipeline wherever the
# method allows it (k = 10, cosine, distance weighting).
#
# Output: results/celeb_bench_<family>_<mode>/celeb_bench_<family>_<mode>_nref..._batch_N.csv
# Same n_ref grid, subsampling scheme and batch seeds as the other benchmark
# scripts, so the reference/test splits match exactly.
#
# Usage:
#   python real_experiment_celeb_openset_bench.py n_ref n_test calib_num alpha_total \
#          n_label_total k_top k_bot batch_num family mode

FAMILIES = ('knn', 'knn1', 'proser', 'openmax', 'occ')
MODES = ('scale', 'center', 'isotonic')

if len(sys.argv) != 11:
    print("Error: incorrect number of parameters.")
    print(
        "Usage: python real_experiment_celeb_openset_bench.py n_ref n_test calib_num "
        "alpha_total n_label_total k_top k_bot batch_num family mode")
    print(f"  family in {FAMILIES}; mode in {MODES}")
    quit()
...
family = sys.argv[9]                # benchmark family (see FAMILIES)
recal_mode = sys.argv[10]           # recalibration mode (see MODES)

if family not in FAMILIES:
    print(f"Error: family must be one of {FAMILIES}, got '{family}'")
    quit()
if recal_mode not in MODES:
...
tag = f"celeb_bench_{family}_{recal_mode}"
...
def make_knn_dist(k_score):
    return black_boxes.KNNDistOpenSet(k_score=k_score, dist_scale=2.0, **KNN_KW)


def make_knn_msp():
    return black_boxes.MSPOpenSet(**KNN_KW)


def make_proser():
    # Identical settings to real_experiment_celeb_proser(_recal).py
    return black_boxes.PROSERMLP(
...
def make_openmax_mlp():
    # Simplified OpenMax: identical settings to real_experiment_celeb_openmax(_recal).py
    return black_boxes.OpenMaxMLP(
        hidden_layer_sizes=(256, 128),
        activation='relu',
        max_iter=500,
        random_state=42,
...
def make_openmax_knn():
    return black_boxes.OpenMaxKNN(
        tail_size=20,
        alpha_rank=None,   # revise all classes
        **KNN_KW
    )
...
# One-class classifiers: 'lof' as in real_experiment_celeb_occ_recal.py;
# 'iforest' and 'ocsvm' exactly as occ_choices in real_experiment_celeb_mm_plugin.py;
# 'ocsvm20' is the narrow-kernel OCSVM.
OCC_SPECS = {
    'lof': dict(occ=LocalOutlierFactor(n_neighbors=1, novelty=True),
                occ_offset=1.0, occ_scale=2.0),
    'iforest': dict(occ=IsolationForest(n_estimators=10, contamination='auto',
                                        random_state=batch_num),
                    occ_offset='auto', occ_scale='auto'),
    'ocsvm': dict(occ=OneClassSVM(kernel='rbf', nu=0.05),
                  occ_offset='auto', occ_scale='auto'),
    'ocsvm20': dict(occ=OneClassSVM(kernel='rbf', nu=0.05, gamma=20.0),
                    occ_offset='auto', occ_scale='auto'),
}


def make_occ(variant):
    spec = OCC_SPECS[variant]
    return black_boxes.OCCOpenSet(occ=spec['occ'], occ_offset=spec['occ_offset'],
                                  occ_scale=spec['occ_scale'], **KNN_KW)


# Methods to evaluate (random internal split only). Method names are the
# CSV row keys read by make_openset_benchmark_table.py and the R plot script.
if family == 'knn':
    methods_list = {
        'Method (Recal KNN-dist k=10)': recal(make_knn_dist(n_neighbors)),
        'Method (Recal KNN-MSP)': recal(make_knn_msp()),
    }
elif family == 'knn1':
    methods_list = {
    ...
    methods_list = {
        'Method (Recal PROSER)': recal(make_proser()),
    }
elif family == 'openmax':
    methods_list = {
        'Method (Recal OpenMax-MLP)': recal(make_openmax_mlp()),
        'Method (Recal OpenMax-KNN)': recal(make_openmax_knn()),
    }
else:  # 'occ'
    methods_list = {}
    for variant in OCC_SPECS:
        methods_list[f'Method (OCC {variant})'] = make_occ(variant)
        methods_list[f'Method (Recal OCC {variant})'] = recal(make_occ(variant))
...
def analyze_data_bench(X_ref, Y_ref, X_test, Y_test, methods_list,
    ...
    Same evaluation as the other benchmark scripts:
...
def run_real_experiment(X, Y, image_names, n_ref, n_test, num_exp, batch_num):
    """
    Runs the experiment `num_exp` times with the same seeds as the other
    benchmark scripts (split seed batch_num * 1000 + i).
    """
...
print(f"  n_ref={n_ref}, n_test={n_test}, alpha_total={alpha_total}, "
      ...
      f"family={family}, mode={recal_mode}")
