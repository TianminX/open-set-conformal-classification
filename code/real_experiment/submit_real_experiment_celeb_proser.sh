...
# Submit SLURM jobs for the RAW PROSER open-set classifier on CelebA
# (Zhou, Ye & Zhan, CVPR 2021: MLP head with classifier placeholders and
# manifold-mixup data placeholders on the FaceNet embeddings), conformalized
# directly like the raw OpenMax runs -- no Good-Turing recalibration.
...
# SLURM parameters (trains a torch MLP for 60 epochs per job on CPU)
...
mkdir -p "results/celeb_proser/"
mkdir -p "logs/celeb_proser/"
        ...
        for N_LABEL_TOTAL in "${N_LABEL_TOTAL_LIST[@]}"; do
          for K_TOP in "${K_TOP_LIST[@]}"; do
            for K_BOT in "${K_BOT_LIST[@]}"; do
              ...
              JOBN="celprosr_n${N_REF}_t${N_TEST}_c${CALIB_NUM}_aT${ALPHA_TOTAL_FMT}_nl${N_LABEL_TOTAL}_k${K_TOP}_${K_BOT}_b${BATCH}"

              # Define output and error log files
              OUTF="logs/celeb_proser/${JOBN}.out"
              ERRF="logs/celeb_proser/${JOBN}.err"

              # Define output CSV file name (must match the Python output path)
              OUT_FILE="results/celeb_proser/celeb_proser_nref${N_REF}_ntest${N_TEST}_cs${CALIB_NUM}_atotal${ALPHA_TOTAL_FMT}_nlabel${N_LABEL_TOTAL}_ktop${K_TOP}_kbot${K_BOT}_batch_${BATCH}.csv"

              if [[ ! -f $OUT_FILE ]]; then
                # If the file doesn't exist, submit the job
                SCRIPT="real_experiment_celeb_proser.sh $N_REF $N_TEST $CALIB_NUM $ALPHA_TOTAL_FMT $N_LABEL_TOTAL $K_TOP $K_BOT $BATCH"
