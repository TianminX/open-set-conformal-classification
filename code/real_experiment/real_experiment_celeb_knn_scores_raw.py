...
# RAW (non-recalibrated) distance/confidence-score open-set classifiers on
# the CelebA real data, conformalized directly like the raw OpenMax runs.
# Same two unknown scores as real_experiment_celeb_knn_scores.py, but taken
# at face value as probabilities, with no Good-Turing level anchoring:
#   KNN-dist: p_unk(x) = cosine distance to the k-th nearest training point,
#             divided by dist_scale=2 (Sun et al., ICML 2022)
#   KNN-MSP:  p_unk(x) = 1 - max_k p_k(x) from the KNN itself
#             (Vaze et al., ICLR 2022)
# Comparing these against their GT-recalibrated counterparts isolates the
# value of the level anchoring, exactly as raw vs recalibrated OpenMax does.
#
...
#
# Usage:
#   python real_experiment_celeb_knn_scores_raw.py n_ref n_test calib_num alpha_total \
#          n_label_total k_top k_bot batch_num

if len(sys.argv) != 9:
    print("Error: incorrect number of parameters.")
    print(
        "Usage: python real_experiment_celeb_knn_scores_raw.py n_ref n_test calib_num "
...
output_file = (
    f"results/celeb_knn_scores_raw/"
    f"celeb_knn_scores_raw_"
    f"nref{n_ref}_"
    f"ntest{n_test}_"
    f"cs{calib_num}_"
...
# Identical base settings to real_experiment_celeb_knn_scores.py, used raw
# (no GTRecalOpenSet wrapper).
classifier_knn_dist = black_boxes.KNNDistOpenSet(
...
classifier_knn_msp = black_boxes.MSPOpenSet(
    n_neighbors=n_neighbors,
    weights='distance',
    algorithm='auto',
    leaf_size=30,
...
methods_list = {
    'Method (KNN-dist)': classifier_knn_dist,
    'Method (KNN-MSP)': classifier_knn_msp,
}
...
def analyze_data_knn_scores_raw(X_ref, Y_ref, X_test, Y_test, methods_list,
                                alpha, calib_size, random_state=2024):
    """
    Run each raw KNN-score method and evaluate prediction sets.
    ...
    for method_name, classifier in methods_list.items():
        ...
        # show the raw (unanchored) unknown mass.
...
def run_real_experiment(X, Y, image_names, n_ref, n_test, num_exp, batch_num):
    """
    Run direct-conformalization experiments with the raw KNN-score classifiers.
    """
    ...
    for i in tqdm(range(num_exp)):
        ...
        results = analyze_data_knn_scores_raw(
