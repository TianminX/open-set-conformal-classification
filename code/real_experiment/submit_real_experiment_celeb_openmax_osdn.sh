...
# Submit SLURM jobs for direct conformalization of OpenMax classifiers on CelebA.
# Baseline comparison for submit_real_experiment_celeb_mm_plugin.sh: same n_ref
# grid, subsampling scheme, and batch seeds so that the reference/test splits
# match the plug-in runs exactly.
...
mkdir -p "results/celeb_openmax_osdn/"
mkdir -p "logs/celeb_openmax_osdn/"
        ...
        for N_LABEL_TOTAL in "${N_LABEL_TOTAL_LIST[@]}"; do
          for K_TOP in "${K_TOP_LIST[@]}"; do
            for K_BOT in "${K_BOT_LIST[@]}"; do
              ...
              JOBN="celosdn_n${N_REF}_t${N_TEST}_c${CALIB_NUM}_aT${ALPHA_TOTAL_FMT}_nl${N_LABEL_TOTAL}_k${K_TOP}_${K_BOT}_b${BATCH}"

              # Define output and error log files
              OUTF="logs/celeb_openmax_osdn/${JOBN}.out"
              ERRF="logs/celeb_openmax_osdn/${JOBN}.err"

              # Define output CSV file name (must match the Python output path)
              OUT_FILE="results/celeb_openmax_osdn/celeb_openmax_osdn_nref${N_REF}_ntest${N_TEST}_cs${CALIB_NUM}_atotal${ALPHA_TOTAL_FMT}_nlabel${N_LABEL_TOTAL}_ktop${K_TOP}_kbot${K_BOT}_batch_${BATCH}.csv"

              if [[ ! -f $OUT_FILE ]]; then
                # If the file doesn't exist, submit the job
                SCRIPT="real_experiment_celeb_openmax_osdn.sh $N_REF $N_TEST $CALIB_NUM $ALPHA_TOTAL_FMT $N_LABEL_TOTAL $K_TOP $K_BOT $BATCH"
