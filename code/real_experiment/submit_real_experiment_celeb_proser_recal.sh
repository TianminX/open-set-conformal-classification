...
# Submit SLURM jobs for the GT-RECALIBRATED PROSER open-set classifier on
# CelebA: the PROSER base (Zhou, Ye & Zhan, CVPR 2021: MLP head with
# classifier placeholders and manifold-mixup data placeholders on the FaceNet
# embeddings) wrapped in the Good-Turing recalibration (unknown column
# rescaled so its out-of-sample mean matches M1/n of the training split).
# Completes the Recal family alongside Recal OpenMax and Recal KNN-dist/MSP.
# Same n_ref grid, subsampling scheme, and batch seeds as
# submit_real_experiment_celeb_proser.sh so the reference/test splits match
# the other runs exactly.
...
# SLURM parameters. The recal wrapper trains the PROSER MLP head twice per
# job (scoring copy on 80% + deployed copy on 100%), so the walltime is
# doubled relative to the raw PROSER runs.
MEMO=8G                             # Memory required
TIME=00-04:00:00                    # Time required
CPUS=4                              # CPUs per task (torch uses OMP threads)
...
mkdir -p "results/celeb_proser_recal/"
mkdir -p "logs/celeb_proser_recal/"
        ...
        for N_LABEL_TOTAL in "${N_LABEL_TOTAL_LIST[@]}"; do
          for K_TOP in "${K_TOP_LIST[@]}"; do
            for K_BOT in "${K_BOT_LIST[@]}"; do
              ...
              JOBN="celprorc_n${N_REF}_t${N_TEST}_c${CALIB_NUM}_aT${ALPHA_TOTAL_FMT}_nl${N_LABEL_TOTAL}_k${K_TOP}_${K_BOT}_b${BATCH}"

              # Define output and error log files
              OUTF="logs/celeb_proser_recal/${JOBN}.out"
              ERRF="logs/celeb_proser_recal/${JOBN}.err"

              # Define output CSV file name (must match the Python output path)
              OUT_FILE="results/celeb_proser_recal/celeb_proser_recal_nref${N_REF}_ntest${N_TEST}_cs${CALIB_NUM}_atotal${ALPHA_TOTAL_FMT}_nlabel${N_LABEL_TOTAL}_ktop${K_TOP}_kbot${K_BOT}_batch_${BATCH}.csv"

              if [[ ! -f $OUT_FILE ]]; then
                # If the file doesn't exist, submit the job
                SCRIPT="real_experiment_celeb_proser_recal.sh $N_REF $N_TEST $CALIB_NUM $ALPHA_TOTAL_FMT $N_LABEL_TOTAL $K_TOP $K_BOT $BATCH"
