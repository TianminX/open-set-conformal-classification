...
# Submit SLURM jobs for the GT-recalibrated KNN-score open-set classifiers
# on CelebA: KNN-dist (k-th nearest-neighbor cosine distance, Sun et al.
# 2022) and KNN-MSP (1 - max KNN probability, Vaze et al. 2022), both with
# the average unknown probability anchored at the Good-Turing estimate M1/n
# of the training split. Few-shot-friendly benchmarks alongside GT-KNN and
# the (recalibrated) OpenMax runs.
...
# SLURM parameters (KNN only; ~4x the GT-KNN work: 2 methods x 2 fits each.
# GT-KNN jobs used ~43 s / ~290 MB.)
...
mkdir -p "results/celeb_knn_scores/"
mkdir -p "logs/celeb_knn_scores/"
        ...
        for N_LABEL_TOTAL in "${N_LABEL_TOTAL_LIST[@]}"; do
          for K_TOP in "${K_TOP_LIST[@]}"; do
            for K_BOT in "${K_BOT_LIST[@]}"; do
              ...
              JOBN="celknnsc_n${N_REF}_t${N_TEST}_c${CALIB_NUM}_aT${ALPHA_TOTAL_FMT}_nl${N_LABEL_TOTAL}_k${K_TOP}_${K_BOT}_b${BATCH}"

              # Define output and error log files
              OUTF="logs/celeb_knn_scores/${JOBN}.out"
              ERRF="logs/celeb_knn_scores/${JOBN}.err"

              # Define output CSV file name (must match the Python output path)
              OUT_FILE="results/celeb_knn_scores/celeb_knn_scores_nref${N_REF}_ntest${N_TEST}_cs${CALIB_NUM}_atotal${ALPHA_TOTAL_FMT}_nlabel${N_LABEL_TOTAL}_ktop${K_TOP}_kbot${K_BOT}_batch_${BATCH}.csv"

              if [[ ! -f $OUT_FILE ]]; then
                # If the file doesn't exist, submit the job
                SCRIPT="real_experiment_celeb_knn_scores.sh $N_REF $N_TEST $CALIB_NUM $ALPHA_TOTAL_FMT $N_LABEL_TOTAL $K_TOP $K_BOT $BATCH"
