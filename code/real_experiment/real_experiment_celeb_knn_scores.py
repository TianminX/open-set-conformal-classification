...
# Distance/confidence-score open-set classifiers on the CelebA real data,
# GT-recalibrated. Two few-shot-friendly unknown scores on top of the same
# KNN closed-set classifier as the CGTC pipeline (seen-class probabilities
# untouched):
#   KNN-dist: s(x) = cosine distance to the k-th nearest training point
#             (Sun et al., ICML 2022)
#   KNN-MSP:  s(x) = 1 - max_k p_k(x) from the KNN itself
#             (Vaze et al., ICLR 2022)
# Both are wrapped in GTRecalOpenSet so the average unknown probability is
# anchored at the Good-Turing estimate M1/n of the training split (out-of-
# sample mean from an internal 20% holdout). Benchmarks alongside GT-KNN
# (constant unknown column) and the recalibrated OpenMax runs.
#
...
#
# Usage:
#   python real_experiment_celeb_knn_scores.py n_ref n_test calib_num alpha_total \
#          n_label_total k_top k_bot batch_num

if len(sys.argv) != 9:
    print("Error: incorrect number of parameters.")
    print(
        "Usage: python real_experiment_celeb_knn_scores.py n_ref n_test calib_num "
...
# k-th nearest-neighbor distance score (Sun et al. 2022): cosine distance to
# the 10th nearest training point. dist_scale=2.0 maps cosine distances into
# [0, 1]; the exact constant is absorbed by the GT recalibration.
base_knn_dist = black_boxes.KNNDistOpenSet(
...
# Maximum-probability score (Vaze et al. 2022): s(x) = 1 - max_k p_k(x)
# from the same KNN.
base_knn_msp = black_boxes.MSPOpenSet(
    n_neighbors=n_neighbors,
    weights='distance',
    algorithm='auto',
    leaf_size=30,
...
classifier_knn_dist = black_boxes.GTRecalOpenSet(
    base=base_knn_dist, recal_frac=0.2, cap=0.9, random_state=42
)
classifier_knn_msp = black_boxes.GTRecalOpenSet(
    base=base_knn_msp, recal_frac=0.2, cap=0.9, random_state=42
)

# Methods to evaluate (random internal split only).
methods_list = {
    'Method (Recal KNN-dist)': classifier_knn_dist,
    'Method (Recal KNN-MSP)': classifier_knn_msp,
}
...
def analyze_data_knn_scores(X_ref, Y_ref, X_test, Y_test, methods_list,
                            alpha, calib_size, random_state=2024):
    """
    Run each recalibrated KNN-score method and evaluate prediction sets.
...
def run_real_experiment(X, Y, image_names, n_ref, n_test, num_exp, batch_num):
    """
    Run direct-conformalization experiments with the recalibrated KNN-score classifiers.
    """
    ...
    for i in tqdm(range(num_exp)):
        ...
        results = analyze_data_knn_scores(
...
header_df = pd.DataFrame({
    ...
    "k_top": [k_top],
    "k_bot": [k_bot],
    "method_family": ["knn_scores_recal"],
})
