...
# RAW PROSER (Zhou, Ye & Zhan, CVPR 2021) on the CelebA real data,
# conformalized directly like the raw OpenMax runs. PROSER trains an MLP
# head on the FaceNet embeddings with classifier placeholders (dummy
# logits) and manifold-mixup data placeholders; the dummy acts as the
# (K+1)-th "unknown" class, so the classifier natively outputs a (K+1)-way
# softmax -- no external unknown score and NO Good-Turing recalibration.
#
...
#
# Usage:
#   python real_experiment_celeb_proser.py n_ref n_test calib_num alpha_total \
#          n_label_total k_top k_bot batch_num

if len(sys.argv) != 9:
    print("Error: incorrect number of parameters.")
    print(
        "Usage: python real_experiment_celeb_proser.py n_ref n_test calib_num "
...
# PROSER MLP head on the FaceNet embeddings. Backbone size matches the
# OpenMax-MLP baseline (256, 128) so the two trained-head methods are
# comparable; n_dummy=5 classifier placeholders as in the original paper.
classifier_proser = black_boxes.PROSERMLP(
...
methods_list = {
    'Method (PROSER)': classifier_proser,
}
...
def analyze_data_proser(X_ref, Y_ref, X_test, Y_test, methods_list,
                        alpha, calib_size, random_state=2024):
    """
    Run each PROSER method and evaluate prediction sets.

    For each method:
      - Call get_prediction_sets_openmax (which does the train/calib split
        internally; PROSER trains its MLP head inside black_box.fit).
...
def run_real_experiment(X, Y, image_names, n_ref, n_test, num_exp, batch_num):
    """
    Run direct-conformalization experiments with the PROSER classifier.
    """
