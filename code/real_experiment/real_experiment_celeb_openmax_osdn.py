...
# the CelebA real data -- FAITHFUL OSDN CONFIGURATION.
#
# Identical to real_experiment_celeb_openmax.py except that the OpenMax
# classifiers follow the OSDN reference implementation (Bendale & Boult, 2016,
# https://github.com/abhijitbendale/OSDN) instead of the simplified variant:
#   - eucos distance (euclidean/200 + cosine) to the class MAVs/centroids,
#   - graded top-alpha revision weights (alpha_rank + 1 - rank)/alpha_rank
#     with alpha_rank = 10, instead of revising all classes at full weight,
#   - OpenMax-MLP revises the raw output-layer logits and then softmaxes over
#     the K+1 revised scores (OpenMax-KNN has no logits, so its revision stays
#     in probability space -- the documented KNN adaptation).
# The per-class Weibull fits keep the pooled fallback for few-shot classes
# (OSDN itself has no fallback; without it most classes would go unrevised in
# this 2-3 images/identity regime).
#
...
#
# Usage:
#   python real_experiment_celeb_openmax_osdn.py n_ref n_test calib_num alpha_total \
#          n_label_total k_top k_bot batch_num

if len(sys.argv) != 9:
    print("Error: incorrect number of parameters.")
    print(
        "Usage: python real_experiment_celeb_openmax_osdn.py n_ref n_test calib_num "
...
output_file = (
    f"results/celeb_openmax_osdn/"
    f"celeb_openmax_osdn_"
    f"nref{n_ref}_"
    f"ntest{n_test}_"
    f"cs{calib_num}_"
...
# embeddings) + OSDN-style Weibull revision on per-class centroid distances
# (eucos distance, graded top-10 alpha weights).
classifier_openmax_knn = black_boxes.OpenMaxKNN(
    ...
    tail_size=20,
    alpha_rank=10,              # OSDN graded top-alpha revision
    distance_type='eucos',      # euclidean/200 + cosine, as in OSDN
    weibull_fallback='pooled'   # few-shot classes fall back to pooled fit
)

# Original OpenMax-MLP (Bendale & Boult, 2016): neural network on the FaceNet
# embeddings + faithful OSDN revision (eucos MAV distances, graded top-10
# alpha weights, logit-space revision + softmax over K+1).
classifier_openmax_mlp = black_boxes.OpenMaxMLP(
    ...
    alpha_rank=10,              # OSDN graded top-alpha revision
    distance_type='eucos',      # euclidean/200 + cosine, as in OSDN
    revision_space='logit',     # revise raw logits, then softmax over K+1
    weibull_fallback='pooled'   # few-shot classes fall back to pooled fit
)
...
header_df = pd.DataFrame({
    ...
    "k_top": [k_top],
    "k_bot": [k_bot],
    "method_family": ["openmax_osdn_direct"],
})
