...
# GT-recalibrated PROSER on the CelebA real data: the PROSER classifier
# (same base as real_experiment_celeb_proser.py) with its unknown column
...
# data). Keeps PROSER's x-dependent unknown ranking but anchors its level;
# completes the Recal family (Recal OpenMax, Recal KNN-dist/KNN-MSP).
#
# Note the recalibration wrapper trains the PROSER MLP head twice per run
# (scoring copy on the 80% subset, deployed copy on the full training
# split), so each job costs roughly double the raw PROSER run.
#
...
#
# Usage:
#   python real_experiment_celeb_proser_recal.py n_ref n_test calib_num alpha_total \
#          n_label_total k_top k_bot batch_num

if len(sys.argv) != 9:
    print("Error: incorrect number of parameters.")
    print(
        "Usage: python real_experiment_celeb_proser_recal.py n_ref n_test calib_num "
...
output_file = (
    f"results/celeb_proser_recal/"
    f"celeb_proser_recal_"
    f"nref{n_ref}_"
    f"ntest{n_test}_"
    f"cs{calib_num}_"
...
# Base PROSER MLP head: identical settings to real_experiment_celeb_proser.py
# so that the recalibrated run differs ONLY by the unknown-column rescaling.
base_proser = black_boxes.PROSERMLP(
...
classifier_recal_proser = black_boxes.GTRecalOpenSet(
    base=base_proser, recal_frac=0.2, cap=0.9, random_state=42
)

# Methods to evaluate (random internal split only).
methods_list = {
    'Method (Recal PROSER)': classifier_recal_proser,
}
...
def analyze_data_proser_recal(X_ref, Y_ref, X_test, Y_test, methods_list,
                              alpha, calib_size, random_state=2024):
    """
    Run each recalibrated-PROSER method and evaluate prediction sets.

    For each method:
      - Call get_prediction_sets_openmax (which does the train/calib split
        internally; the recal wrapper trains the PROSER MLP head inside
        black_box.fit -- once on the 80% scoring subset, once on the full
        training split).
    ...
    for method_name, classifier in methods_list.items():
        ...
        # p_open AUC is identical to the raw PROSER run; the p_open means
...
def run_real_experiment(X, Y, image_names, n_ref, n_test, num_exp, batch_num):
    """
    Run direct-conformalization experiments with the recalibrated PROSER classifier.
    """
    ...
    for i in tqdm(range(num_exp)):
        ...
        results = analyze_data_proser_recal(
