...
# Missing mass correction flag (0 = off, 1 = on)
ADJUST_ALPHA_LIST=(0 1)
...
mkdir -p "results/dp_tuned_mixed_labels_mm/rebuttal/"
mkdir -p "logs/dp_mm/"
          ...
          for ALPHA_TOTAL in "${ALPHA_TOTAL_LIST[@]}"; do
            for LAMBDA_WEIGHT in "${LAMBDA_WEIGHT_LIST[@]}"; do
              for TUNING_METHOD in "${TUNING_METHOD_LIST[@]}"; do
                for ADJUST_ALPHA in "${ADJUST_ALPHA_LIST[@]}"; do
                ...
                JOBN="dp_mm_theta${THETA}_n${N_REF}_t${N_TEST}_c${CALIB_NUM}_aT${ALPHA_TOTAL_FMT}_l${LAMBDA_WEIGHT_FMT}_tm${TUNING_METHOD}_aa${ADJUST_ALPHA}_b${BATCH}"

                # Define output and error log files
                OUTF="logs/dp_mm/${JOBN}.out"
                ERRF="logs/dp_mm/${JOBN}.err"

                # Check for existing output (matches Python's output path)
                OUT_FILE_FMT="results/dp_tuned_mixed_labels_mm/rebuttal/dp_occ%s_betacv_theta%s_nref${N_REF}_ntest${N_TEST}_cs${CALIB_NUM}_atotal${ALPHA_TOTAL_FMT}_lambda${LAMBDA_WEIGHT_FMT}_tune${TUNING_METHOD}_adjalpha${ADJUST_ALPHA}_batch${BATCH}.csv"
