...
if len(sys.argv) != 7:
    ...
    print("Usage: python synthetic_experiment_evm.py theta n_ref n_test calib_num alpha_total batch_num")
    quit()
...
# EVM-KNN: KNN base classifier + EVM margin-based Weibull (K+1 output)
classifier_evm_knn = black_boxes.EVMKNN(
...
# Methods to evaluate
methods_list = {
    'Method (EVM-KNN)': classifier_evm_knn,
    'Method (Bernoulli EVM-KNN)': classifier_evm_knn,
}
...
def analyze_data_evm(X_ref, Y_ref, X_test, Y_test, methods_list,
                     alpha, calib_size, random_state=2024):
    """
    Run each EVM method and evaluate prediction sets.
    Same structure as analyze_data_openmax in synthetic_experiment_openmax.py.
    """
    ...
    for method_name, classifier in methods_list.items():
        ...
        # Coverage for calib-not-train test points
...
def run_evm_experiment(n_ref, n_test, num_exp, batch_num):
...
results = run_evm_experiment(n_ref, n_test, num_exp, batch_num)
