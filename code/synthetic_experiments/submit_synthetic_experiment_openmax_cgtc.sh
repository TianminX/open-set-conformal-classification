        ...
        for ALPHA_TOTAL in "${ALPHA_TOTAL_LIST[@]}"; do
          for LAMBDA_WEIGHT in "${LAMBDA_WEIGHT_LIST[@]}"; do
            for TUNING_METHOD in "${TUNING_METHOD_LIST[@]}"; do
              ...
              JOBN="dp_omcgtc_theta${THETA}_n${N_REF}_t${N_TEST}_c${CALIB_NUM}_aT${ALPHA_TOTAL_FMT}_l${LAMBDA_WEIGHT_FMT}_tm${TUNING_METHOD}_b${BATCH}"

              # Define output and error log files
              OUTF="logs/dp_openmax_cgtc/${JOBN}.out"
              ERRF="logs/dp_openmax_cgtc/${JOBN}.err"
              ...
              if [[ -f "$OUT_FILE_INT" || -f "$OUT_FILE_DOT" ]]; then
                ...
                SCRIPT="synthetic_experiment_openmax_cgtc.sh $THETA $N_REF $N_TEST $CALIB_NUM $ALPHA_TOTAL_FMT $LAMBDA_WEIGHT_FMT $BATCH $TUNING_METHOD"
