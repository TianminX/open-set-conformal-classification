...
mkdir -p "results/dp_gt_knn/"
mkdir -p "logs/dp_gt_knn/"
    ...
    for N_REF in "${N_REF_LIST[@]}"; do
      for N_TEST in "${N_TEST_LIST[@]}"; do
        ...
        for ALPHA_TOTAL in "${ALPHA_TOTAL_LIST[@]}"; do
          ...
          JOBN="dp_gt_knn_theta${THETA}_n${N_REF}_t${N_TEST}_c${CALIB_NUM}_aT${ALPHA_TOTAL_FMT}_b${BATCH}"

          # Define output and error log files
          OUTF="logs/dp_gt_knn/${JOBN}.out"
          ERRF="logs/dp_gt_knn/${JOBN}.err"

          # Define the base pattern once
          OUT_FILE_FMT="results/dp_gt_knn/dp_theta%s_nref${N_REF}_ntest${N_TEST}_cs${CALIB_NUM}_atotal${ALPHA_TOTAL_FMT}_batch${BATCH}.csv"
          ...
          if [[ -f "$OUT_FILE_INT" || -f "$OUT_FILE_DOT" ]]; then
            ...
            SCRIPT="synthetic_experiment_gt_knn.sh $THETA $N_REF $N_TEST $CALIB_NUM $ALPHA_TOTAL_FMT $BATCH"
