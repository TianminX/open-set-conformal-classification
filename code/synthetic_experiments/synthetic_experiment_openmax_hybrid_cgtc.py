...
# Fixed mode:  python synthetic_experiment_openmax_hybrid_cgtc.py theta n_ref n_test calib_num alpha_total lambda_weight batch_num -1 alpha_class alpha_unseen alpha_seen

if len(sys.argv) < 9:
    print("Error: incorrect number of parameters.")
    print("Usage (tuned):  python synthetic_experiment_openmax_hybrid_cgtc.py theta n_ref n_test calib_num alpha_total lambda_weight batch_num tuning_method_flag")
    print("Usage (fixed):  python synthetic_experiment_openmax_hybrid_cgtc.py theta n_ref n_test calib_num alpha_total lambda_weight batch_num -1 alpha_class alpha_unseen alpha_seen")
    quit()
...
if tuning_method_flag == 0:
    ...
    if len(sys.argv) != 12:
        ...
        print("Usage: python synthetic_experiment_openmax_hybrid_cgtc.py theta n_ref n_test calib_num alpha_total lambda_weight batch_num -1 alpha_class alpha_unseen alpha_seen")
        quit()
...
output_file = (
    f"results/dp_openmax_hybrid_cgtc/"
    f"dp_"
    f"theta{theta}_"
    f"nref{n_ref}_"
    f"ntest{n_test}_"
...
# OpenSetKNNwithMLPOpenMax: KNN for p_seen + MLP-based OpenMax for p_open
...
def run_experiment(n_ref, n_test, num_exp, batch_num):
    ...
    classifiers_dict = {
        'OpenMax-Hybrid': classifier_hybrid,
    }
