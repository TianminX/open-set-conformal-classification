...
# Fixed mode:  python synthetic_experiment_gt_knn_openmax_hybrid_cgtc.py theta n_ref n_test calib_num alpha_total lambda_weight batch_num -1 alpha_class alpha_unseen alpha_seen

if len(sys.argv) < 9:
    print("Error: incorrect number of parameters.")
    print("Usage (tuned):  python synthetic_experiment_gt_knn_openmax_hybrid_cgtc.py theta n_ref n_test calib_num alpha_total lambda_weight batch_num tuning_method_flag")
    print("Usage (fixed):  python synthetic_experiment_gt_knn_openmax_hybrid_cgtc.py theta n_ref n_test calib_num alpha_total lambda_weight batch_num -1 alpha_class alpha_unseen alpha_seen")
    quit()
...
if tuning_method_flag == 0:
    ...
    if len(sys.argv) != 12:
        ...
        print("Usage: python synthetic_experiment_gt_knn_openmax_hybrid_cgtc.py theta n_ref n_test calib_num alpha_total lambda_weight batch_num -1 alpha_class alpha_unseen alpha_seen")
        quit()
...
output_file = (
    f"results/dp_gt_knn_openmax_hybrid_cgtc/"
    f"dp_"
    f"theta{theta}_"
    f"nref{n_ref}_"
    f"ntest{n_test}_"
...
# OpenSetKNNwithGTKNNOpenMaxHybrid: KNN for p_seen + GT-anchored KNN-OpenMax modulation for p_unseen
classifier_gt_knn_openmax = black_boxes.OpenSetKNNwithGTKNNOpenMaxHybrid(
    calibrate=False,
...
def run_experiment(n_ref, n_test, num_exp, batch_num):
    ...
    classifiers_dict = {
        'GT-KNN-OpenMax-Hybrid': classifier_gt_knn_openmax,
    }
    ...
    for i in tqdm(range(num_exp)):
        ...
        if tuning_method_flag != -1:
            alpha_class_tuned, alpha_unseen_tuned, alpha_seen_tuned, tuning_results = tune_alpha_allocation_loss_all_optimized(
                ...
                n_splits=10,
                alpha_step=0.005,
                classifier=classifier_gt_knn_openmax,
                occ=occ,
                calibration_probability=calib_prob_adjusted,
                calib_size=calib_size,
                pvalue_method='XGT',
