...
# Submit script for synthetic_experiment_openmax_cgtc.py with FIXED alpha allocation
# Usage: bash submit_synthetic_experiment_openmax_cgtc_fixed.sh
...
mkdir -p "results/dp_openmax_cgtc/"
mkdir -p "logs/dp_openmax_cgtc_fixed/"
      ...
      for N_TEST in "${N_TEST_LIST[@]}"; do
        ...
        for ALPHA_TOTAL in "${ALPHA_TOTAL_LIST[@]}"; do
          for LAMBDA_WEIGHT in "${LAMBDA_WEIGHT_LIST[@]}"; do
            ...
            JOBN="dp_omcgtc_fixed_theta${THETA}_n${N_REF}_t${N_TEST}_c${CALIB_NUM}_aT${ALPHA_TOTAL_FMT}_ac${ALPHA_CLASS}_au${ALPHA_UNSEEN}_as${ALPHA_SEEN}_b${BATCH}"

            # Define output and error log files
            OUTF="logs/dp_openmax_cgtc_fixed/${JOBN}.out"
            ERRF="logs/dp_openmax_cgtc_fixed/${JOBN}.err"

            # Define the base pattern once
            OUT_FILE_FMT="results/dp_openmax_cgtc/dp_theta%s_nref${N_REF}_ntest${N_TEST}_cs${CALIB_NUM}_atotal${ALPHA_TOTAL_FMT}_lambda${LAMBDA_WEIGHT_FMT}_tune${TUNING_METHOD}_batch${BATCH}.csv"
            ...
            if [[ -f "$OUT_FILE_INT" || -f "$OUT_FILE_DOT" ]]; then
              ...
              SCRIPT="synthetic_experiment_openmax_cgtc.sh $THETA $N_REF $N_TEST $CALIB_NUM $ALPHA_TOTAL_FMT $LAMBDA_WEIGHT_FMT $BATCH $TUNING_METHOD $ALPHA_CLASS $ALPHA_UNSEEN $ALPHA_SEEN"
