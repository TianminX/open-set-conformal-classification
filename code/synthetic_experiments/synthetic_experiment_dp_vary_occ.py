...
# OCC sensitivity rerun (response letter, Referee 1 Q5): identical to
# synthetic_experiment_dp.py (CV-selected beta) except that the one-class
# classifier is a command-line argument and the iforest/ocsvm choices use the
# configurations described in the response letter (100-tree Isolation Forest on
# the full sample; RBF OCSVM with adaptive bandwidth).
#
# Parse command-line arguments
# Tuned mode:  python synthetic_experiment_dp_vary_occ.py theta n_ref n_test calib_num alpha_total lambda_weight batch_num tuning_method_flag occ_name
# Fixed mode:  python synthetic_experiment_dp_vary_occ.py theta n_ref n_test calib_num alpha_total lambda_weight batch_num -1 alpha_class alpha_unseen alpha_seen occ_name

if len(sys.argv) < 10:
    print("Error: incorrect number of parameters.")
    print("Usage (tuned):  python synthetic_experiment_dp_vary_occ.py theta n_ref n_test calib_num alpha_total lambda_weight batch_num tuning_method_flag occ_name")
    print("Usage (fixed):  python synthetic_experiment_dp_vary_occ.py theta n_ref n_test calib_num alpha_total lambda_weight batch_num -1 alpha_class alpha_unseen alpha_seen occ_name")
    quit()
...
if tuning_method_flag == 0:
    ...
    tuning_method = 'fixed'
    if len(sys.argv) != 13:
        print("Error: fixed mode requires 3 extra arguments before occ_name: alpha_class alpha_unseen alpha_seen")
        print("Usage: python synthetic_experiment_dp_vary_occ.py theta n_ref n_test calib_num alpha_total lambda_weight batch_num -1 alpha_class alpha_unseen alpha_seen occ_name")
        quit()
...
# One-class classifier choice is the last argument: 'lof', 'iforest', 'ocsvm'
occ_name = sys.argv[12] if tuning_method_flag == -1 else sys.argv[9]
...
output_file = (
    f"results/dp_tuned_mixed_labels/vary_occ/"
    f"dp_"
    f"occ{occ_name}_"
    f"{beta_label}_"
    f"theta{theta}_"
...
class AdaptiveBandwidthOCSVM:
    """RBF One-Class SVM whose kernel bandwidth gamma is fitted from the
    training data at fit time, using the median nearest-neighbor distance d
    (gamma = 1/(2 d^2), i.e. the kernel decays at the local scale of the
    training sample). Duck-types the fit/score_samples API used by
    compute_XGT_pvalues_testing_new, so it can be refit repeatedly during
    ...
    def fit(self, X):
        ...
        if not np.isfinite(d) or d <= 0:
            # Fall back to the smallest positive distance (duplicate rows)
            positive = dists[dists > 0]
            d = np.min(positive) if positive.size > 0 else 1.0
        self.gamma_ = 1.0 / (2.0 * d ** 2)
        self.model_ = OneClassSVM(kernel='rbf', nu=self.nu, gamma=self.gamma_)
        self.model_.fit(X)
        return self
...
print(f"  n_ref={n_ref}, n_test={n_test}, alpha_total={alpha_total}, "
      ...
      f"batch_num={batch_num}, occ={occ_name}")
