...
if tuning_method_flag == 0:
    ...
    # adjust_alpha_flag is optional, at position 9
    adjust_alpha_flag = int(sys.argv[9]) if len(sys.argv) > 9 else 0
    ...
    # adjust_alpha_flag is optional, at position 12
    adjust_alpha_flag = int(sys.argv[12]) if len(sys.argv) > 12 else 0
...
adjust_alpha = (adjust_alpha_flag == 1)
...
print(f"adjust_alpha (missing mass correction): {adjust_alpha} (flag={adjust_alpha_flag})")
...
output_file = (
    f"results/dp_tuned_mixed_labels_mm/rebuttal/"
    f"dp_"
    f"occ{occ_name}_"
    f"{beta_label}_"
    f"theta{theta}_"
    ...
    f"adjalpha{adjust_alpha_flag}_"
...
def compute_mu_hat(Y):
    """
    Compute the Good-Turing missing mass estimate.

    mu_hat = M_1 / n, where M_1 is the number of singleton labels
    (labels appearing exactly once) and n is the total number of points.

    Returns 0 if mu_hat >= 1 (degenerate case: all labels are singletons).
    """
    n = len(Y)
    if n == 0:
        return 0.0
    ...
    mu_hat = M_1 / n
    if mu_hat >= 1.0:
        return 0.0  # degenerate case: fall back to no adjustment
    return mu_hat


def adjust_alpha_class_fn(alpha_class, mu_hat):
    """
    Inflate alpha_class by the missing mass correction factor.

    Returns alpha_class / (1 - mu_hat), capped at 1.0.
    If mu_hat is 0 (or was reset to 0 due to degeneracy), returns alpha_class unchanged.
    """
    if mu_hat <= 0.0:
        return alpha_class
    alpha_adj = alpha_class / (1.0 - mu_hat)
    return min(alpha_adj, 1.0)
...
def analyze_data(X_ref, Y_ref, X_test, Y_test, methods_list,
    ...
    If adjust_alpha is True, alpha_class is inflated by the missing mass correction
    factor for non-benchmark methods. Benchmark methods use the full alpha_total budget
    ...
    # Compute missing mass correction
    mu_hat = compute_mu_hat(Y_ref)
    if adjust_alpha:
        alpha_class_adj = adjust_alpha_class_fn(alpha_class, mu_hat)
        tqdm.write(f"Missing mass correction: mu_hat={mu_hat:.4f}, "
                   f"alpha_class={alpha_class:.4f} -> alpha_class_adj={alpha_class_adj:.4f}")
    else:
        alpha_class_adj = alpha_class
    ...
    for method_name, method_function in methods_list.items():
        ...
        if method_name == 'Method (benchmark)' or method_name == 'Method (benchmark full)':
            # Benchmark uses full alpha_total budget — no missing mass adjustment
            ...
            # Use adjusted alpha_class for non-benchmark methods
            decoded_prelim_sets = method_function(
                X_ref, Y_ref, X_test,
                alpha_prime=alpha_class_adj,
                black_box=classifier,
                calibration_probability=calib_prob_adjusted,  # Use adjusted function
                random_state=random_state
            )
        ...
        for pvalue_method in ['GT', 'XGT', 'RGT']:
            ...
            if method_name in ['Method (benchmark)', 'Method (Bernoulli benchmark)', 'Method (benchmark full)',
                ...
                final_sets = finalize_prediction_sets(
                    ...
                    X_ref, Y_ref, X_test,
                    pvalue_method,
                    alpha_unseen, alpha_seen,  # Pass both alpha parameters (unchanged)
                    occ=occ,
                    random_state=random_state,
                    beta=best_beta
                )
...
def run_syn_experiment(n_ref, n_test, num_exp, batch_num):
    ...
    for i in tqdm(range(num_exp)):
        ...
        # Analyze with tuned alphas (missing mass adjustment applied inside analyze_data)
...
header_df = pd.DataFrame({
    ...
    "adjust_alpha": [adjust_alpha],
    "adjust_alpha_flag": [adjust_alpha_flag]
})
...
print(f"  n_ref={n_ref}, n_test={n_test}, alpha_total={alpha_total}, "
      ...
      f"batch_num={batch_num}, adjust_alpha={adjust_alpha}")
